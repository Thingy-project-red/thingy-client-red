...
export class TvocGraphComponent implements OnInit {
  ...
  */
  ...
  loadGraphData(timeRange, interval) {
    ...
    (error) => {
      this.errorService.addError('TVOC graph: could not load data for thingy1', new Date());
      console.log(error);
    });
    ...
    (error) => {
      this.errorService.addError('TVOC graph: could not load data for thingy2', new Date());
      console.log(error);
    });
  }
