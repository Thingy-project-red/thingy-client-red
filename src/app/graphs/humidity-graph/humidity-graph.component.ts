...
export class HumidityGraphComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.loadGraphData('humidity', 3600, '');
  }

  loadGraphData(metric, timeRange, interval) {

    this.dataLoaded = false;

    this.graphDataService.getData('Thingy1', 'humidity', 3600, '').subscribe((data) => {
      this.graphData1 = data;
      let counter = 0;
      let average = 0;
      for(let i = 0; i<this.graphData1.length; i++) {
        if(counter<60) {
          average = average + this.graphData1[i].humidity;
          counter++;
        } else {
    ...
    this.graphDataService.getData('Thingy2', 'humidity', 3600, '').subscribe((data) => {
      this.graphData2 = data;
      let counter = 0;
      let average = 0;
      for(let i = 0; i<this.graphData2.length; i++) {
        if(counter<60) {
          average = average + this.graphData2[i].humidity;
          counter++;
        } else {
